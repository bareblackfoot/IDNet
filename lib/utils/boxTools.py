...
class DPP():
    def __init__(self, sim_classes=[], epsilon=0.01, loss_weight=0.001):
    ...
    def dpp_greedy(self, S, scores_s, score_power, max_per_image, among_ims, num_gt_per_img=1000, close_thr=0.0001):
        ...
        while (len(keep) < max_per_image) and sum(left[:, 1]) > 0:
            ...
            # ====================================================================
            if len(keep) > 0:
                prob_rest[left_indices] = - np.sum(
                    np.multiply(np.dot(S[left_indices, :][:, keep], d_1), S[left_indices, :][:, keep]), 1)
    ...
    def dpp_MAP(self, im_dets_pair, scores, boxes, sim_classes, score_thresh, epsilon, max_per_image,
                close_thr=0.00001):
        """
        DPP MAP inference
        """
    ...
    def compute_kernel(self, labels, boxes, Phi, loc_argmax, unnormalized_bbox_targets, im_shape_w, im_shape_h, sim_feat):
        """
        ...
        sim_images = sim_feat * sim_images
    ...
    def compute_log_p(self, Y, S_y, y, Phi, det_L_I, M, Phi_power):
        ...
        if len(Y) == 0:
            ...
            log_p = 2 * Phi_power * np.sum(np.multiply(y, np.reshape(np.log(Phi), (M, 1)))) + np.log(
                np.linalg.det(S_y)) - np.log(det_L_I)
        return log_p
    ...
    def Compute_Xy(self, Y, keeps_Y, labels, pred_boxes):
        ...
        for label_k in dict_keeps_Y.keys():
            ...
            max_ol_ll = np.max(np.reshape(IoU_target(pbox, Y_selected_pbox), (len(idxs_k), len(dict_keeps_Y[label_k]))),
                               1)
    ...
    def compute_diff_logp(self, labels_Xy, Xy, y, exp_cls_score, bbox_pred, boxes, Phi, unnormalized_bbox_targets,
        ...
        IoU, S, L, IoU_with_gt_all, pred_boxes, det_L_I = self.compute_kernel(labels_Xy, boxes[Xy, :], Phi,
                                                                              loc_argmax,
                                                                              unnormalized_bbox_targets[Xy, :],
                                                                              im_shape_w, im_shape_h)
        M_y = len(Xy)
        Adj_L = np.linalg.inv(L + np.eye(M_y)) * det_L_I
        diag_L = np.diag(L)
        Kii = np.reshape(np.repeat(diag_L, K), (M_y, K)) / det_L_I
        Phi_frac = np.divide(exp_cls_score[Xy, :],
                             np.reshape(np.repeat(np.sum(exp_cls_score[Xy, 1:], 1), K), (M_y, K)) + 0.0001)
